...
// ------------------------------         Locators Ends here !          ----------------------------------------------------
	...
	public Boolean editEntityfromgridView(String astr_entityName) throws Throwable 
	{
		try 
		{
			if (getGridView().isDisplayed()) 
			{
				...
				CommonBean.lbase_baseClass.explicitElementToBeClickable(GridEntityName);
	...
	public void createNewGroupingRule(String astr_groupingRuleName, String astr_Description, String astr_RuleDefinition) throws Throwable 
	{
		try 
		{
			CommonBean.lbase_baseClass.clickButton(getClickNewGroupingRuleBtn());

			String getDynamicRule = CommonBean.lRule_Engine_Definition_ruleDefinition.createGroupingRule(astr_groupingRuleName, astr_Description, astr_RuleDefinition);
	...
	public void navigate_to_Workflow(String astr_workflowName) throws Throwable 
	{
		try 
		{
			...
			CommonBean.lbase_baseClass.explicitElementToBeClickable(gridWorkflowName);
			CommonBean.lbase_baseClass.doubleClick(gridWorkflowName);
			//Thread.sleep(3000);
			
			CommonBean.lbase_baseClass.explicitVisibilityOf(getWorkflowLoader());			
			CommonBean.lbase_baseClass.explicitInvisibilityOfElementLocated(getWorkflowLoader());		
			try 
			{
				CommonBean.lbase_baseClass.explicitTextToBePresentInElement(gridWorkflowName, astr_workflowName);
			} catch (Exception e) 
			{
				// TODO Auto-generated catch block
				//e.printStackTrace();
